...
/** Tiny Z overlap so boolean subtract is manifold-friendly (mm). */
const CUTTER_Z_EPS = 0.02;

/**
 * Font basenames (no extension) allowed when productMode is "stencil" — OFL faces from Google Fonts (see README).
 * Normalization: lowercase, strip non-alphanumeric for comparison.
 */
const STENCIL_FONT_NAME_PARTS = [
  "stardosstencilbold",
  "stardosstencilregular",
  "allertastencilregular",
  "sairastenciloneregular",
  "blackopsoneregular",
];
...
export function isStencilAllowedFontName(name) {
  const key = normalizeFontKey(name);
  return STENCIL_FONT_NAME_PARTS.includes(key);
}

/**
 * @param {object} params
 * @param {"stencil"|"form"} productMode
 */
export function assertStencilFontIfNeeded(params, productMode) {
  if (String(productMode || "").toLowerCase() !== "stencil") return;
  const raw =
    params.fontName != null
      ? String(params.fontName)
      : params.fontPath != null
        ? path.basename(String(params.fontPath))
        : params.fontUrl != null
          ? (String(params.fontUrl).split("/").pop() || "").split("?")[0]
          : "";
  if (!raw) return;
  if (!isStencilAllowedFontName(raw)) {
    throw new Error(
      `Stencil mode only allows OFL stencil fonts bundled with the project (e.g. StardosStencil-Bold). Got "${raw}".`
    );
  }
}
...
/**
 * @param {object} params - { rectangleWidth, rectangleHeight, thickness, text, productMode?, pocketDepth?, letterHeight? (deprecated: use pocketDepth for form), textAlign?, resolution?, fontPath?, fontUrl?, padding?, addStake?, stakeWidth?, stakeHeight? }
 */
export async function generate(params, options = {}) {
  ...
  const productMode = String(params.productMode ?? "stencil").toLowerCase();
  if (productMode !== "stencil" && productMode !== "form") {
    throw new Error('productMode must be "stencil" or "form".');
  }
  let cutDepth;
  if (productMode === "stencil") {
    cutDepth = thickness;
  } else {
    const pocket = params.pocketDepth != null ? params.pocketDepth : params.letterHeight;
    cutDepth = toFiniteNumber(pocket ?? 1, "pocketDepth");
    if (cutDepth <= 0 || cutDepth > thickness) {
      throw new Error("pocketDepth must satisfy 0 < pocketDepth ≤ thickness.");
    }
  }
  const cutterHeight = cutDepth + CUTTER_Z_EPS;
  ...
  if (rectangleWidth <= 0 || rectangleHeight <= 0 || thickness <= 0) {
    throw new Error("rectangleWidth, rectangleHeight, thickness must be > 0.");
  }
  ...
  if (font) {
    const out = getTextContoursFromFont(font, text, rectangleWidth, rectangleHeight, 0, padding, { ...options, textAlign, resolution });
  ...
  for (let gi = 0; gi < glyphContours.length; gi++) {
    ...
    for (let k = 0; k < outers.length; k++) {
      ...
      for (const tri of triangles) {
        ...
        const extruded = extrudeLinear({ height: cutterHeight }, g2);
        const raised = translate([0, 0, thickness - cutDepth - CUTTER_Z_EPS], extruded);
        cutters.push(raised);
      }
    }
  }
  ...
  let geometry =
    cutters.length === 0 ? plate : subtract(plate, cutters.length === 1 ? cutters[0] : union(...cutters));
  if (addStake) {
  ...
  return {
    ...
    meta: {
      ...
      productMode,
      cutDepth,
      pocketDepth: productMode === "form" ? cutDepth : undefined,
      text,
      name,
      textAlign,
      resolution,
...
/**
 * Generate Font-to-STL: text cut from a stock plate (stencil through-cut or form pocket).
 *
 * @param {object} params - {
 *   text: string,
 *   mode: "separate" | "combined",
 *   productMode: "stencil" | "form",
 *   characterHeight: number (mm),
 *   spacing: number (mm),
 *   plateThickness: number (mm),
 *   platePadding: number (mm),
 *   pocketDepth: number (mm, form mode only — depth of cut from top face),
 *   fontPath?: string,
 *   fontUrl?: string,
 *   fontName?: string,
 *   resolution?: number
...
export async function generateFontToSTL(params, options = {}) {
  ...
  const plateThickness = toFiniteNumber(params.plateThickness ?? 2, "plateThickness");
  const platePadding = toFiniteNumber(params.platePadding ?? 2, "platePadding");
  const resolution = params.resolution != null ? Math.max(2, Math.min(128, Math.round(Number(params.resolution)))) : DEFAULT_RESOLUTION;

  if (plateThickness <= 0 || characterHeight <= 0) {
    throw new Error("plateThickness and characterHeight must be > 0");
  }

  let cutDepth;
  if (productMode === "stencil") {
    cutDepth = plateThickness;
  } else {
    const pocket = params.pocketDepth != null ? params.pocketDepth : params.cutDepth;
    cutDepth = toFiniteNumber(pocket ?? 1, "pocketDepth");
    if (cutDepth <= 0 || cutDepth > plateThickness) {
      throw new Error("pocketDepth must satisfy 0 < pocketDepth ≤ plateThickness.");
    }
  }

  const cutterHeight = cutDepth + CUTTER_Z_EPS;

  assertStencilFontIfNeeded(params, productMode);
  ...
  if (options.debug) {
    console.error(`[debug] mode=${mode} productMode=${productMode} letters=${letters.length} text="${text}" cutDepth=${cutDepth}`);
  }
  ...
  if (mode === "separate") {
    ...
    for (const letter of letters) {
      const geom = extrudeLetterContours(letter.contours, cutterHeight, options.debug);
      if (!geom) continue;

      const plateWidth = letter.bounds.width + 2 * platePadding;
      const plateHeight = letter.bounds.height + 2 * platePadding;
      const cx = letter.bounds.minX + letter.bounds.width / 2;
      const cy = letter.bounds.minY + letter.bounds.height / 2;
      const plate = translate([cx, cy, plateThickness / 2], cuboid({ size: [plateWidth, plateHeight, plateThickness] }));
      const cutter = translate([0, 0, plateThickness - cutDepth - CUTTER_Z_EPS], geom);
      const finalGeom = subtract(plate, cutter);
  ...
  let cutterUnion = geometries.length === 1 ? geometries[0] : union(...geometries);

  const shiftUp = -minY;
  if (Math.abs(shiftUp) > 0.001) {
    cutterUnion = translate([0, shiftUp, 0], cutterUnion);
    maxY = maxY + shiftUp;
    minY = 0;
  }
  ...
  const plate = translate([plateCenterX, plateCenterY, plateThickness / 2], cuboid({ size: [plateWidth, plateHeight, plateThickness] }));
  const cutterRaised = translate([0, 0, plateThickness - cutDepth - CUTTER_Z_EPS], cutterUnion);
  const finalGeom = subtract(plate, cutterRaised);
