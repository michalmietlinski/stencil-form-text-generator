...
function assertStencilFontIfNeeded(params, productMode) {
  if (String(productMode || "").toLowerCase() !== "stencil") return;
  let raw = params.fontName != null ? String(params.fontName) : "";
  if (!raw && params.fontPath) raw = fontBasename(params.fontPath);
  if (!raw && params.fontUrl) raw = fontBasename(params.fontUrl);
  if (!raw) return;
